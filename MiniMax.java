public class MiniMax implements IOthelloAI {
    ...
    // Helper method equivalent to max(), but allows for returning the best move found
    ...
    // This method represents the max player and returns the evaluation of the best move found
    private int max(GameState s, int depth, int a, int b) {
        ...
        // If there are no legal moves, pass the turn to the other player and continue searching.
        if(moves.size() == 0){
            s.changePlayer();
        ...
        for (Position move : moves) {
            ...
            // If the current depth is less than the maximum depth, continue the search with min player.
            // Otherwise, evaluate the current game state using the evaluation function.
            var value = depth < maxDepth ? 
                min(copy, depth, Math.max(a, bestValue), b) : 
                eval(s);

            // Update the best value if the new value is greater than the current best value.
            if(value > bestValue){
                bestValue = value;
            }
            // Perform beta.
    ...
    // This method represents the min player and returns the evaluation of the best move found
    private int min(GameState s, int depth, int a, int b){
        ...
        for (Position move : moves) {
            ...
            // If the current depth is less than the maximum depth, continue the search with min player.
            // Otherwise, evaluate the current game state using the evaluation function.
            int value = depth < maxDepth ? 
                max(copy, depth, a, Math.min(b, bestValue)) : 
                eval(s);

            // Update the best value if the new value is smaller than the current best value.
            if(value < bestValue){
                bestValue = value;
            }
    ...
    private int eval(GameState s){
        ...
        // calculate token evaluation score based on difference between number of tokens of each color
    ...
    // A private helper method that returns a value based on whether a token is of the current player's color, the opponent's color, or neither
